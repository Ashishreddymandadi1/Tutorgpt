...
interface HealthResponse {
  ...
  service: string
  version: string
  ai_service: AiServiceHealth | { status: string; error: string }
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        ...
        {data && (
          <>
            <div className="space-y-4 mb-8">
              <ServiceRow name="Frontend" description="React + Vite (this page)" status="ok" />
        ...
        <button
          onClick={() => refetch()}
          className="mt-6 w-full rounded-lg border border-gray-200 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors"
        >
          Refresh
        </button>
      </div>
